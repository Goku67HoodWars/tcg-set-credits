...
```powershell
# interactive
& ([scriptblock]::Create((irm https://raw.githubusercontent.com/Goku67HoodWars/tcg-set-credits/main/tcg-set-credits.ps1)))

# direct
& ([scriptblock]::Create((irm https://raw.githubusercontent.com/Goku67HoodWars/tcg-set-credits/main/tcg-set-credits.ps1))) -Credits 5692500 -Yes
```

It always pulls the current `main`, so pushing an update makes the next run use it
automatically. (Plain `iex` won't work because the script has a `param()` block —
...
function Set-TcgCredits {
  & ([scriptblock]::Create((irm https://raw.githubusercontent.com/Goku67HoodWars/tcg-set-credits/main/tcg-set-credits.ps1))) @args
}
```
